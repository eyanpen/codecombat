...
exports.setupMiddleware = function (app) {
  ...
  // Mount database API routes (before proxy so they work locally)
  if (!config.proxy) {
    const dbRoutes = require('./server/routes-db')
    const authRoutes = require('./server/routes-auth')
    const userDataRoutes = require('./server/routes-user-data')
  ...
  // Catch-all: serve main.html for any unmatched route (SPA front-end routing)
  if (!config.proxy) {
    const wrap = require('co-express')
    app.get('*', wrap(function * (req, res) {
      const template = yield getStaticTemplate('main.html')
